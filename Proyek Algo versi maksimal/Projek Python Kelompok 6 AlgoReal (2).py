...
def catat_log(username, aksi):
    tambah_baris(FILE_LOG, ["username","action","date"],
                 {"username": username, "action": aksi, "date": "now"})
...
def kelola_komoditas():
    while True:
        ...
        if pilihan == "1":
            data = ambil_komoditas()
            if len(data) == 0:
                print("   → Belum ada komoditas terdaftar.\n")
            else:
                cetak_tabel(data, [("commodity_id","ID"), ("name","Nama"), ("unit","Satuan")])
